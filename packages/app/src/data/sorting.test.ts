...
const TEST_DATA =
  ...
  'a,a,100,01.01.2022,false,3\n' +
  'b,d,1e1,01.01.2022,true,1\n' +
  'c,u,NaN,02.01.2021,false,2\n' +
  'd,f,,,,\n' +
  'e,b,-,-,-,-'
...
describe('sorting', () => {
  ...
  it('stableSort', () => {
    ...
    expect(stableSort(data.allEntities, { type: 'asc', column: stringColumn })[0].uid as string).toEqual('a')
    expect(stableSort(data.allEntities, { type: 'asc', column: numberColumn })[0].uid as string).toEqual('b')
    expect(stableSort(data.allEntities, { type: 'asc', column: dateColumn })[0].uid as string).toEqual('c')
    expect(stableSort(data.allEntities, { type: 'asc', column: booleanColumn })[0].uid as string).toEqual('b')
    expect(stableSort(data.allEntities, { type: 'asc', column: timestampColumn })[0].uid as string).toEqual('b')
  })
})
