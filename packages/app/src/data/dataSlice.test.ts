...
const selectData = (s: RootState): LoadedData => {
  if (s.data.type === 'loading-complete') {
    ...
    throw new Error('Illegal data loading state')
  }
}
...
describe('dataSlice', () => {
  ...
  it('loadData loading-complete', async () => {
    ...
    expect(patientData.allEntities[0].pid as string).toEqual('PID_1')
    expect(patientData.allEntities[1].pid as string).toEqual('PID_2')
    ...
    expect(eventData.allEntities[0].eid as string).toEqual('EID_1')
    expect(eventData.allEntities[0].pid as string).toEqual('PID_1')
    expect(eventData.allEntities[1].eid as string).toEqual('EID_2')
    expect(eventData.allEntities[1].pid as string).toEqual('PID_2')
  })
  ...
  it(`handles ${setIndexPatient.type} action`, async () => {
    ...
    expect(getIndexPatientId() as string).toEqual('PID_1')
